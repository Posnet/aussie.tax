...
function updateTax() {
  const inputElem = document.getElementById("income");
  const income = parseFloat(inputElem.value);
        ...
        if (index > 0) {
          ...
          if (band.start !== currentBand.start) {
            startSpan.style.color =
              band.start < currentBand.start
                ? "var(--highlight-bad)"
                : "var(--highlight-good)";
          }
          if (band.end !== currentBand.end) {
            endSpan.style.color =
              band.end > currentBand.end
                ? "var(--highlight-good)"
                : "var(--highlight-bad)";
          }
          if (band.rate !== currentBand.rate) {
            rateSpan.style.color =
              band.rate > currentBand.rate
                ? "var(--highlight-bad)"
                : "var(--highlight-good)";
          }
        }
...
function formatCurrency(amount) {
  let result = `$${amount.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, "$&,")}`;
